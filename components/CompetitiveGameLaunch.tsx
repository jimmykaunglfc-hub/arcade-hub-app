...
export default function CompetitiveGameLaunch({ gameKey, gameTitle, Game, onClose, preloadedRoomId, allowLocal = true }: { gameKey: string; gameTitle: string; Game: ComponentType<GameProps>; onClose: () => void; preloadedRoomId?: string | null; allowLocal?: boolean }) {
  ...
  // Closing the arena now enters the app-wide 30-second return window. Do not
  // record an immediate loss here: a player who simply tapped Back can resume
  // the still-mounted match without being penalized.
  if (opponent) return <Game onClose={onClose} onResult={reportResult} roomId={room?.id} seat={room?.seat ?? matchedSeat ?? undefined} matchId={matchedId || room?.id} currentUserId={userId} opponent={opponent} />;
  if (matchError) return <div className="fixed inset-0 z-[100] grid place-items-center bg-background p-6 text-center text-on-background"><div><p className="font-bold">{matchError}</p><button onClick={onClose} className="mt-5 rounded-xl bg-primary px-5 py-3 text-xs font-black text-on-primary"><LocalizedText id="UI_0276" fallback="Back to arcade" /></button></div></div>;
  if (foundMatch && (["bingo", "four-in-a-row", "dominoes", "ping-pong"].includes(gameKey))) return <div className="fixed inset-0 z-[150] flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[#08090b] p-6 text-center text-white"><div className="w-full max-w-sm"><div className="mx-auto mb-16 inline-flex items-center gap-2 rounded-full border border-[#ccff00]/35 bg-[#ccff00]/10 px-5 py-3 text-xs font-black uppercase tracking-[.16em] text-[#ccff00]"><span>✦</span> <LocalizedText id="UI_0278" fallback="Match Confirmed" /></div><div className="relative mx-auto mb-10 flex h-28 w-48 items-center justify-center"><div className="absolute left-1 top-3 grid h-24 w-24 -rotate-6 place-items-center rounded-[25px] border border-white/15 bg-[#17171b] text-3xl text-neutral-400 shadow-2xl">♙</div><div className="absolute right-1 top-3 grid h-24 w-24 rotate-6 place-items-center rounded-[25px] border border-indigo-400/50 bg-indigo-500/20 text-3xl text-indigo-300 shadow-2xl">♙</div><div className="z-10 grid h-12 w-12 place-items-center rounded-full bg-[#ccff00] text-3xl text-black">×</div></div><p className="text-[11px] font-black uppercase tracking-[.22em] text-neutral-500"><LocalizedText id="UI_0279" fallback="Opposing Player" /></p><h1 className="mt-2 truncate text-4xl font-black tracking-tight">{foundMatch.name}</h1><p className="mt-3 text-lg text-neutral-400"><span className="mr-2 text-[#ccff00]">●</span>{foundMatch.isBot ? tr("UI_0091", "Arena opponent") : tr("UI_0280", "Ranked")} <LocalizedText id="UI_0277" fallback="· 1200 ELO" /></p><button onClick={() => void enterRoomBackedMatch()} className="mt-16 w-full rounded-[24px] bg-[#ccff00] py-5 text-2xl font-black text-black shadow-[0_14px_36px_rgba(204,255,0,.2)] active:scale-[.98]"><LocalizedText id="UI_0093" fallback="Enter Match&nbsp; →" /></button></div></div>;
  ...
  if (mode === "host") return <div className="fixed inset-0 z-[100] grid place-items-center bg-[#09090b] p-6 text-center text-white"><div className="w-full max-w-sm rounded-[32px] border border-white/10 bg-[#18181b] p-7"><p className="text-xs font-bold uppercase tracking-widest text-neutral-400"><LocalizedText id="UI_0283" fallback="Create a private room" /></p><button onClick={async()=>{if(!await reserveEntry("Guest Player"))return;const {data:{user}}=await supabase.auth.getUser(); const {data,error}=await supabase.rpc("create_two_player_room",{p_game_key:gameKey,p_name:user?.email?.split("@")[0]||"Player 1",p_state:{}}); if(error){setMatchError(error.message);return;} if(gameKey==="four-in-a-row") await supabase.rpc("create_four_in_a_row_state",{p_room_id:data.room_id}); setRoom({id:data.room_id,seat:1});setMatchedId(data.room_id);setRoomCode(data.room_code);}} className="mt-6 w-full rounded-2xl bg-[#CCFF00] py-3 text-xs font-black uppercase text-black"><LocalizedText id="UI_0284" fallback="Create room" /></button>{roomCode&&<><p className="mt-6 text-xs font-bold uppercase text-neutral-400"><LocalizedText id="UI_0288" fallback={tr("UI_0288", "Share this room code")} /></p><p className="my-3 font-mono text-4xl font-black tracking-[.2em] text-[#CCFF00]">{roomCode}</p>{["bingo", "ping-pong", "dominoes"].includes(gameKey) ? <p className="rounded-2xl bg-white/5 py-3 text-xs font-black uppercase text-neutral-300"><LocalizedText id="UI_0289" fallback={tr("UI_0289", "Waiting for the other player…")} /></p> : <button onClick={()=>setOpponent({name:"Online Player",isBot:false})} className="w-full rounded-2xl bg-white py-3 text-xs font-black uppercase text-black"><LocalizedText id="UI_0286" fallback={tr("UI_0286", "Enter room")} /></button>}</>}<button onClick={() => setMode("menu")} className="mt-4 text-xs font-bold text-neutral-400"><LocalizedText id="UI_0281" fallback="Back" /></button></div></div>;
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-background p-6 text-center text-on-background"><div><p className="font-bold">{status}</p><button onClick={onClose} className="mt-5 rounded-xl bg-primary px-5 py-3 text-xs font-black text-on-primary"><LocalizedText id="UI_0276" fallback="Back to arcade" /></button></div></div>;
}
