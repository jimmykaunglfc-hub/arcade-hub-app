...
export default function CompetitiveGameLaunch({ gameKey, gameTitle, Game, onClose, preloadedRoomId }: { gameKey: string; gameTitle: string; Game: ComponentType<GameProps>; onClose: () => void; preloadedRoomId?: string | null }) {
  ...
  useEffect(() => { void (async () => {
    if (!preloadedRoomId || !["bingo", "four-in-a-row", "dominoes"].includes(gameKey)) return;
  ...
  useEffect(() => {
    if (!["bingo", "dominoes"].includes(gameKey) || !room || opponent || foundMatch) return;
    let mounted = true;
  ...
  const startMatch = async (match: { opponent: { name: string; isBot: boolean } }) => {
    if (!await reserveEntry(match.opponent.name)) return;
  ...
  if (opponent) return <Game onClose={() => { void reportResult("Loss"); onClose(); }} onResult={reportResult} roomId={room?.id} seat={room?.seat} />;
