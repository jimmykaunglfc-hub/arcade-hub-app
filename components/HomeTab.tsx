...
export default function HomeTab({ 
  ...
  useEffect(() => {
    ...
    const setupSubscriptions = async () => {
      ...
      profileChannel = supabase
        ...
        .on(
          "postgres_changes",
          { event: "UPDATE", schema: "public", table: "profiles", filter: `id=eq.${activeUserId}` },
      ...
      matchesChannel = supabase
        ...
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "match_history", filter: `user_id=eq.${activeUserId}` },
          () => { fetchUserDataAndMatches(); }
        ).subscribe();
    };
  ...
  const stats = useMemo(() => {
    ...
    const favorite = Object.keys(gameCounts).length > 0 
  ...
  const displayedGroups = showAllGames ? groupedMatches : groupedMatches.slice(0, 5);
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            <div className="w-14 h-14 rounded-full bg-secondary-container flex items-center justify-center shadow-sm">
              <span className="material-symbols-outlined text-secondary text-[24px]">donut_large</span>
            </div>
          ...
          <div className="w-full animate-fade-in">
            <div className="flex items-center justify-between mb-4 mt-2">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-headline font-black text-on-surface">{selectedGame} History</h2>
              </div>
              <span className="text-xs text-on-surface-variant font-bold">
                {groupedMatches.find(g => g[0] === selectedGame)?.[1].length} Plays
              </span>
            </div>

            <div className="flex flex-col gap-3">
              {groupedMatches.find(g => g[0] === selectedGame)?.[1].map((match, i) => (
                <div key={match.id || i} className="bg-surface border border-surface-container-highest rounded-2xl p-4 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${
                      match.isVictory ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                      match.isDraw ? "bg-amber-500/10 border-amber-500/20 text-amber-400" :
                      "bg-rose-500/10 border-rose-500/20 text-rose-400"
                    }`}>
                      <span className="material-symbols-outlined text-lg">
                        {match.isVictory ? "emoji_events" : match.isDraw ? "balance" : "close"}
                      </span>
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-on-surface mb-0.5">vs {match.opponentName || "Unknown"}</h4>
                      <p className="text-[11px] text-on-surface-variant font-medium">
                        <span className={match.isVictory ? "text-emerald-400" : match.isDraw ? "text-amber-400" : "text-rose-400"}>
                          {match.result}
                        </span>
                  ...
                  <span className="text-[10px] text-on-surface-variant font-medium">{match.timeAgo}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {groupedMatches.length === 0 ? (
                ...
                displayedGroups.map(([gameTitle, gameMatches], i) => {
                  ...
                  const gameWins = gameMatches.filter(m => m.isVictory).length;
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-surface-container-high rounded-[14px] flex items-center justify-center border border-surface-container-highest group-hover:scale-105 transition-transform shrink-0">
                          <span className="material-symbols-outlined text-xl text-primary">{iconName}</span>
                        </div>
                        <div>
                          <h3 className="font-headline font-bold text-base text-on-surface mb-0.5">{gameTitle}</h3>
        ...
        <div className="fixed inset-0 z-[999] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in touch-none">
          <div className="bg-[#18181b] border border-white/10 rounded-[32px] p-6 w-full max-w-[340px] shadow-2xl relative">
            ...
            <div className="flex flex-col items-center mb-6 pt-4">
              ...
              <h2 className="font-headline font-black text-xl text-white uppercase tracking-tight">Player Statistics</h2>
              <p className="text-xs text-neutral-400 font-medium mt-1">Lifetime gameplay record</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col items-center text-center shadow-inner">
                <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">Win Rate</span>
                <span className="text-2xl font-black text-[#CCFF00]">{stats.winRate}%</span>
              </div>
              <div className="bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col items-center text-center shadow-inner">
                <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">Matches</span>
                <span className="text-2xl font-black text-white">{stats.total}</span>
              </div>
              <div className="bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col items-center text-center shadow-inner">
                <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">Victories</span>
                <span className="text-xl font-black text-blue-400">{stats.wins}</span>
              </div>
              <div className="bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col items-center text-center justify-center shadow-inner">
                <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">Top Game</span>
                <span className="text-sm font-bold text-white truncate w-full">{stats.favorite}</span>
              </div>
            </div>
