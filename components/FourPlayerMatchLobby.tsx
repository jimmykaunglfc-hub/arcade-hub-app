...
export default function FourPlayerMatchLobby({ gameKey, gameName, userId, onStart, onCancel }: { gameKey: string; gameName: string; userId: string; onStart: (roomId: string) => void; onCancel: () => void }) {
  ...
  useEffect(() => { if (!roomId) return; const refresh = async () => { const { data } = await supabase.rpc("get_matchmaking_room", { p_room_id: roomId }); if (!data || !alive.current) return; const players = data.players || []; const humans = players.filter((p: Seat) => !p.is_bot); const left = Math.max(0, Math.ceil((new Date(data.expires_at).getTime() - Date.now()) / 1000)); if (left === 0 && data.status === "waiting" && humans.length >= 2) { await supabase.rpc("fill_expired_four_player_bots", { p_room_id: roomId }); return; } setSeats(players); setSeconds(left); setExpired(left === 0 && humans.length < 2); if (data.status === "starting" && data.host_id === userId && players.length === 4 && players.every((p: Seat) => p.ready) && !dealt.current) { dealt.current = true; const rpc = gameKey === "big-two" ? "big_two_deal_room" : "start_four_player_room"; const { error } = await supabase.rpc(rpc, { p_room_id: roomId }); if (error) dealt.current = false; } if (data.status === "playing") onStart(roomId); }; void refresh(); const id = window.setInterval(refresh, 1000); return () => clearInterval(id); }, [gameKey, onStart, roomId, userId]);
