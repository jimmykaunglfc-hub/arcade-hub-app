...
export default function Game2048Battle({ onClose }: Game2048BattleProps) {
 ...
 return (
   <div className="absolute inset-x-0 bottom-20 top-0 flex min-h-0 flex-col overflow-hidden bg-[radial-gradient(circle_at_top,#4c1d95_0%,#1e1b4b_48%,#020617_100%)] text-white select-none">
     <header className="grid shrink-0 grid-cols-[1fr_auto_1fr] items-center gap-2 border-b border-white/10 bg-slate-950/55 px-3 py-2 backdrop-blur">
       <div className="justify-self-start">
         {onClose && <button type="button" onClick={onClose} aria-label="Back to Arcade Hub" className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-400/30 bg-slate-900/85 active:scale-95"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-5 w-5"><path d="M19 12H5"/><path d="m12 19-7-7 7-7"/></svg></button>}
       </div>
       <div className="text-center"><h1 className="text-lg font-black text-amber-300">2048 BATTLE</h1><p className="text-[9px] font-black uppercase tracking-[0.18em] text-violet-200/70">You vs Computer</p></div>
       <div className="flex items-center justify-self-end gap-2">
         <button type="button" onClick={reset} className="rounded-xl bg-amber-400 px-3 py-2 text-xs font-black text-slate-950 active:scale-95">New</button>
         <button type="button" onClick={() => setShowRules(true)} aria-label="How to play 2048 Battle" className="flex h-9 w-9 items-center justify-center rounded-full border border-[#ccff00] bg-slate-900 text-[#ccff00] shadow-[0_0_12px_rgba(204,255,0,0.25)] active:scale-95"><span className="flex h-5 w-5 items-center justify-center rounded-full border border-[#ccff00] text-xs font-black">?</span></button>
       </div>
     </header>
         ...
         <section className="grid grid-cols-[1fr_auto] items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/60 p-3 shadow-xl">
           <div className="grid h-full min-h-24 grid-cols-2 gap-2">
             <div className="flex flex-col items-center justify-center rounded-2xl border border-cyan-300/15 bg-cyan-400/10 px-2 py-3 text-center shadow-inner">
               <span className="text-[9px] font-black uppercase tracking-wider text-cyan-300">Your score</span>
             ...
             <div className="flex flex-col items-center justify-center rounded-2xl border border-rose-300/15 bg-rose-400/10 px-2 py-3 text-center shadow-inner">
               <span className="text-[9px] font-black uppercase tracking-wider text-rose-300">Computer</span>
               <strong className="mt-1 text-3xl leading-none text-white">{computerScore}</strong>
             </div>
           </div>
           <div className="w-24"><p className="mb-1 text-center text-[8px] font-black uppercase text-rose-300">Live AI board</p><Grid board={computerBoard} compact /></div>
         </section>

         <section>
           <div className="mb-2 flex items-center justify-between"><span className="text-xs font-black uppercase tracking-widest text-cyan-300">Your board</span><span className="rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold">Best {highestTile(yourBoard)}</span></div>
           <div
     ...
     {showRules && (
       <div className="fixed inset-x-0 bottom-20 top-14 z-[250] flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md">
         <div role="dialog" aria-modal="true" aria-labelledby="battle-2048-rules" className="max-h-[92%] w-full max-w-md overflow-y-auto rounded-[2rem] border-2 border-[#ccff00] bg-gradient-to-b from-slate-900 to-slate-950 p-5 shadow-[0_0_35px_rgba(204,255,0,0.18)]">
           <div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#ccff00]">2048 Battle</p><h2 id="battle-2048-rules" className="text-2xl font-black">How to Play</h2></div><button type="button" onClick={() => setShowRules(false)} aria-label="Close how to play" className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-500 bg-slate-800 text-2xl font-black">×</button></div>
           <div className="mt-5 space-y-3">
             {[["👆","Swipe to move","Swipe your board up, down, left, or right. All tiles move together."],["➕","Merge matching tiles","Two equal tiles combine into one larger tile and add to your score."],["🤖","Real computer board","After every valid move, the computer evaluates and moves its own visible board."],["⚖️","One move each","You and the computer receive one move per turn. Invalid swipes do not cost a turn."],["🏆","Reach 2048 first","The first side to create a 2048 tile wins the battle."],["🔒","Locked board","If a board has no moves, scores and highest tiles decide the winner."]].map(([icon,title,text]) => <div key={title} className="flex gap-3 rounded-2xl border border-slate-700 bg-slate-800/90 p-3.5"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-950 text-xl">{icon}</span><div><h3 className="font-black text-amber-300">{title}</h3><p className="mt-1 text-sm leading-5 text-slate-300">{text}</p></div></div>)}
           </div>
           <button type="button" onClick={() => setShowRules(false)} className="mt-5 w-full rounded-2xl bg-gradient-to-b from-amber-400 to-amber-500 py-3.5 font-black uppercase text-slate-950 shadow-[0_5px_0_#c56b00] active:translate-y-1 active:shadow-none">Got It — Let&apos;s Play</button>
         </div>
       </div>
     )}

     {winner && (
       <div className="absolute inset-0 z-[260] flex items-center justify-center bg-slate-950/85 p-5 backdrop-blur-md"><div className="w-full max-w-sm rounded-[2rem] border-2 border-amber-300 bg-slate-900 p-7 text-center shadow-2xl"><div className="text-6xl">{winner === "you" ? "🏆" : winner === "draw" ? "🤝" : "🤖"}</div><h2 className="mt-3 text-3xl font-black">{winner === "you" ? "You Win!" : winner === "draw" ? "Draw Battle" : "Computer Wins"}</h2><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-cyan-500/15 p-3"><span className="block text-[9px] uppercase text-cyan-300">You</span><strong className="text-xl">{yourScore}</strong></div><div className="rounded-xl bg-rose-500/15 p-3"><span className="block text-[9px] uppercase text-rose-300">Computer</span><strong className="text-xl">{computerScore}</strong></div></div><button type="button" onClick={reset} className="mt-6 w-full rounded-2xl bg-amber-400 py-3 font-black text-slate-950">Play Again</button></div></div>
     )}
