...
export default function ChessGame({ onClose, preloadedMatchId }: ChessGameProps) {
  ...
  const capturedPieces = useMemo(() => {
    ...
    for (const type of ["q", "r", "b", "n", "p"] as const) {
      for (let i = 0; i < starting[type] - counts.w[type]; i++) {
        wCaptured.push(PIECE_SYMBOLS[type.toUpperCase()]);
      }
      for (let i = 0; i < starting[type] - counts.b[type]; i++) {
      ...
      for (let c = 0; c < 8; c++) {
        ...
        if (piece && piece.type === "k" && piece.color === game.turn()) {
          ...
          checkSquares[`${file}${rank}`] = {
            background:
              "radial-gradient(circle, rgba(255, 0, 0, 0.8) 0%, rgba(255, 0, 0, 0.4) 100%)",
          ...
          <div className="w-full space-y-3">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-headline font-bold text-sm tracking-wide text-neutral-200">
                  LOCAL PASS & PLAY
                </span>
              </div>
              <span className="material-symbols-outlined text-lg opacity-50">
      ...
      <div className="fixed inset-0 z-[100] bg-[#09090b] flex flex-col font-body text-white">
        <div className="flex justify-between items-center p-6 bg-gradient-to-b from-black/50 to-transparent">
          ...
          <div className="text-center">
            <h2 className="font-headline font-black text-sm uppercase tracking-widest">
              Chess Matrix
            </h2>
            <div className="flex items-center justify-center gap-1.5 mt-1">
              <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
              <span className="font-caps text-[9px] font-bold tracking-widest text-indigo-400">
                CONNECTING...
              </span>
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-[100] bg-[#09090b] flex flex-col items-center justify-center font-body text-white">
        ...
        <div className="absolute inset-0 z-[200] bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-6 animate-fade-in">
          <div className="w-full max-w-[340px] bg-[#18181b] border border-white/10 rounded-[32px] p-8 flex flex-col items-center text-center shadow-2xl">
            ...
            <button
              ...
              className="w-full bg-white/5 hover:bg-white/10 text-neutral-300 rounded-xl py-4 font-headline font-bold text-sm tracking-widest border border-white/5 transition-colors"
            >
              EXIT ARENA
            </button>
          </div>
      ...
      <div className="w-full max-w-[400px] flex items-start justify-between px-4 pt-safe absolute top-0 mt-4 z-10">
        ...
        <div className="flex flex-col items-center">
          ...
          <span
            className={`font-caps text-[9px] font-bold uppercase tracking-widest mt-0.5 ${
              matchId && !opponentConnected
      ...
      <div className="w-full max-w-[400px] flex flex-col gap-4 px-4 w-full pt-10">
        <div
          className={`w-full bg-[#18181b] border rounded-2xl p-3 flex flex-col relative transition-all duration-300 shadow-lg ${
            ...
            (!matchId && currentTurnColor === "white")
              ? "border-white/5"
        ...
        >
          ...
          <div className="w-full flex flex-wrap gap-1 mt-2 min-h-[20px] text-lg opacity-70 px-1">
            {(matchId
              ? playerColor === "white"
                ? capturedPieces.wCaptured
                : capturedPieces.bCaptured
              : capturedPieces.wCaptured
            ).map((p, i) => (
              <span key={i} className="leading-none text-white">
                {p}
              </span>
            ))}
          </div>
