...
export default function ShanKoeMeeGame({ roomId, onClose, onPlayAgain }: { roomId?: string | null; onClose?: () => void; onPlayAgain?: () => void }) {
  ...
  return <section className="fixed inset-0 z-[100] min-h-[100dvh] overflow-hidden bg-[#041208] pt-[var(--app-safe-top)] text-white">
    <header className="relative z-20 flex h-14 items-center justify-between border-b border-amber-300/25 bg-slate-950/90 px-3 backdrop-blur"><button onClick={onClose} className="grid size-9 place-items-center rounded-full bg-white/10" aria-label={tr("UI_0276", "Back to arcade")}>←</button><div className="text-center"><h1 className="text-sm font-black text-amber-300">{language === "en" ? tr("UI_1335", "ရှမ်းကိုးမီး · Shan Koe Mee") : "ရှမ်းကိုးမီး"}</h1><p className="text-[9px] font-bold uppercase tracking-widest text-emerald-200/70"><LocalizedText id="UI_1334" fallback="Four-player traditional table · Round" />{round}</p></div><button onClick={() => setLanguage((value) => value === "en" ? "my" : "en")} className="rounded-lg border border-amber-300/30 px-2 py-1 text-[10px] font-black text-amber-200">{language === "en" ? "မြန်မာ" : tr("UI_1333", "EN")}</button></header>
    <main className="relative mx-auto h-[calc(100dvh-3.5rem-var(--app-safe-top))] max-w-4xl p-3"><div className="relative h-full overflow-hidden rounded-[42px] border-[7px] border-[#251406] bg-[radial-gradient(circle_at_center,#168144_0%,#0b542c_58%,#042d17_100%)] shadow-[inset_0_0_90px_rgba(0,0,0,.85)]"><div className="pointer-events-none absolute inset-0 grid place-items-center text-center text-2xl font-black tracking-[.35em] text-white/[.055]"><LocalizedText id="UI_1337" fallback="SHAN KOE MEE" /><br/><span className="text-xs"><LocalizedText id="UI_1336" fallback="4 PLAYER TABLE" /></span></div>
      {orderedSeats.map((seat) => <div key={seat.seat} className={`absolute z-10 flex flex-col items-center ${position(seat.seat)}`}><div className={`mb-1 rounded-full px-2 py-1 text-[10px] font-black ${seat.seat === banker ? "bg-amber-400 text-slate-950" : "bg-slate-950/85 text-white"}`}>{seat.seat === banker ? tr("UI_1338", "👑 Banker ·") : ""}{seat.seat === mySeat ? tr("UI_0084", "You") : seat.name} · {chips[seat.seat] ?? 1000}</div><div className="flex -space-x-2">{hands[seat.seat].length ? hands[seat.seat].map((card, index) => <PlayingCard key={`${card.rank}-${card.suit}-${index}`} card={card} hidden={seat.seat !== mySeat && !revealed} />) : <><PlayingCard hidden /><PlayingCard hidden /></>}</div>{(seat.seat === mySeat || revealed) && hands[seat.seat].length > 0 && <span className="mt-1 rounded-full bg-black/55 px-2 py-0.5 text-[9px] font-bold text-amber-100">{label(hands[seat.seat])}</span>}</div>)}
      <div className="absolute left-1/2 top-1/2 z-10 w-[min(88%,360px)] -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-amber-200/25 bg-slate-950/85 p-4 text-center shadow-2xl"><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#ccff00]"><LocalizedText id="UI_1349" fallback="Traditional table" /></p><p className="mt-2 text-sm font-bold text-white">{message}</p>{secondsLeft !== null && <p className="mt-2 text-xs font-black text-amber-300"><LocalizedText id="UI_1339" fallback={tr("UI_1339", "Auto action in")} />{secondsLeft}s</p>}{phase === "banker" && <div className="mt-4 grid grid-cols-4 gap-2">{orderedSeats.map((seat) => <div key={seat.seat} className="text-center"><PlayingCard card={bankerDraw[seat.seat]} /><span className="mt-1 block text-[9px]">{seat.seat === mySeat ? tr("UI_0084", "You") : seat.name}</span></div>)}</div>}{phase === "betting" && <><div className="mt-4 flex justify-center gap-2">{[50,100,300].map((amount) => <button key={amount} onClick={() => setBet(amount)} className={`rounded-full px-4 py-2 text-xs font-black ${bet === amount ? "bg-[#ccff00] text-black" : "bg-white/10 text-white"}`}>{amount}</button>)}</div><button onClick={deal} className="mt-4 w-full rounded-2xl bg-amber-400 py-3 text-sm font-black text-slate-950"><LocalizedText id="UI_1343" fallback={tr("UI_1343", "Deal cards")} /></button></>}{phase === "choice" && <div className="mt-4 grid grid-cols-2 gap-2"><button onClick={draw} className="rounded-2xl bg-[#ccff00] py-3 text-sm font-black text-black"><LocalizedText id="UI_1344" fallback={tr("UI_1344", "Draw card")} /></button><button onClick={stay} className="rounded-2xl bg-white/10 py-3 text-sm font-black"><LocalizedText id="UI_1348" fallback={tr("UI_1348", "Stay")} /></button></div>}{phase === "result" && <div className="mt-4 grid grid-cols-2 gap-2"><button onClick={nextRound} className="rounded-2xl bg-[#ccff00] py-3 text-sm font-black text-black"><LocalizedText id="UI_1346" fallback={tr("UI_1346", "Next round")} /></button><button onClick={onPlayAgain ?? onClose} className="rounded-2xl bg-white/10 py-3 text-sm font-black"><LocalizedText id="UI_1345" fallback={tr("UI_1345", "Exit table")} /></button></div>}</div>
    </div></main>
  </section>;
}
