...
function BuildingMarkers({ level, ownerColor }: { level: number; ownerColor: string }) {
  ...
  return (
    <span aria-label={isHotel ? tr("UI_1106", "Hotel") : level > 0 ? `${level} house${level === 1 ? "" : "s"}` : tr("UI_1107", "Owned property")} className={`absolute right-[5%] top-[5%] z-20 grid h-[clamp(11px,2.7vw,18px)] w-[clamp(11px,2.7vw,18px)] place-items-center rounded-full border border-white/90 shadow-[0_0_7px_rgba(255,255,255,.5),0_1px_3px_rgba(0,0,0,.75)] ${isHotel ? "bg-[#8e2c20] text-[#fff4c7]" : "bg-[#061018]/85 text-[#c9ffb8]"}`} style={{ boxShadow: `0 0 7px ${ownerColor}, 0 1px 3px rgba(0,0,0,.75)` }}>
...
function BoardTile({ space, tokens, owner, mortgaged = false, activePlayerId, colorBandEdge = "top", onInspect }: { space: BoardSpace; tokens: Player[]; owner?: Player; mortgaged?: boolean; activePlayerId: string; colorBandEdge?: "top" | "right" | "bottom" | "left"; onInspect?: (spaceId: string) => void }) {
  ...
  return (
    <div role={canInspect ? "button" : undefined} tabIndex={canInspect ? 0 : undefined} onClick={canInspect ? () => onInspect?.(space.id) : undefined} onKeyDown={canInspect ? (event) => { if (event.key === "Enter" || event.key === " ") onInspect?.(space.id); } : undefined} className={`relative flex min-h-0 min-w-0 flex-col items-center justify-center overflow-hidden border border-[#20242a] bg-[linear-gradient(135deg,#fffef6_0%,#e4e3dc_62%,#b7b4a8_100%)] px-px py-px text-center text-[#11151a] shadow-[inset_0_1px_0_rgba(255,255,255,.95),inset_0_-2px_0_rgba(34,35,36,.22)] ${isOwned ? "z-10" : ""} ${canInspect ? "cursor-pointer focus-visible:z-30 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white" : ""}`} style={{ background: mortgaged ? "linear-gradient(135deg,#37434b,#111920 60%,#344957)" : owner?.color ?? specialBackground, color: specialTextColor, boxShadow: glow }}>
      ...
      {mortgaged && <span className="absolute inset-x-0 top-1/2 -translate-y-1/2 rotate-[-18deg] border-y border-white/35 bg-[#091017]/80 py-px text-[clamp(3px,.85vw,6px)] font-black tracking-[.1em] text-white"><LocalizedText id="UI_1111" fallback={tr("UI_1111", "MORTGAGED")} /></span>}
      ...
      {isPricedTile ? (
        ...
        <>
          ...
          {space.kind === "tax" && <span className="mt-px text-[clamp(4px,1.05vw,7px)] font-black"><LocalizedText id="UI_1113" fallback={tr("UI_1113", "PAY $")} />{space.taxAmount}</span>}
        </>
      )}
  ...
  return (
    <section className="col-start-2 col-end-9 row-start-2 row-end-9 relative flex min-h-0 min-w-0 items-center justify-center overflow-hidden bg-[radial-gradient(ellipse_at_50%_43%,#1e4d72_0%,#0a1a2a_52%,#03070e_100%)]">
      ...
      <div className="absolute top-[6%] z-30 flex min-w-[48%] max-w-[86%] flex-col items-center rounded-full border border-white/15 bg-[#06121ed9] px-[5%] py-[2%] text-center shadow-[0_0_16px_rgba(72,177,255,.2),inset_0_1px_0_rgba(255,255,255,.12)]">
        <span className="text-[clamp(4px,1.15vw,7px)] font-black uppercase tracking-[.18em] text-[#8fdcff]"><LocalizedText id="UI_1117" fallback="CRYSTAL TABLE - ACTIVE TURN" /></span>
        ...
        <time dateTime={`PT${secondsLeft}S`} className={`mt-[3%] text-[clamp(5px,1.3cqw,8px)] font-black tracking-[.14em] ${turnWarning ? "text-[#ffe28a]" : "text-[#ddf7ff]"}`}><LocalizedText id="UI_1118" fallback="TURN TIMER ·" />{timerLabel}</time>
      ...
      {showStartBanner && <div className="pointer-events-none absolute left-1/2 top-[17%] z-40 w-[76%] -translate-x-1/2 rounded-lg border border-[#98dcff]/80 bg-[#061322]/95 px-[4%] py-[2.5%] text-center shadow-[0_0_18px_rgba(70,190,255,.28),inset_0_1px_0_rgba(255,255,255,.18)]"><p className="text-[clamp(5px,1.35vw,9px)] font-black uppercase tracking-[.08em] text-white"><LocalizedText id="UI_1119" fallback={tr("UI_1119", "ALL PLAYERS ARE ON GO.")} /></p><p className="mt-[1%] text-[clamp(6px,1.55vw,11px)] font-black uppercase tracking-[.04em] text-[#ffdb45]"><LocalizedText id="UI_1120" fallback={tr("UI_1120", "ROLL TO START THE JOURNEY")} /></p></div>}

      <button type="button" onClick={isEndTurn ? onEndTurn : onRoll} disabled={!canRoll && !isEndTurn} aria-label={tr("UI_1121", "Roll two dice on the white stage")} className="absolute left-1/2 top-[55%] z-40 h-[34%] w-[60%] overflow-visible rounded-[25%] outline-none transition hover:scale-105 focus-visible:ring-2 focus-visible:ring-[#ffe18a] disabled:cursor-not-allowed" style={{ transform: "translate(-50%, -50%)" }}>
        ...
        <span className="absolute -bottom-[24%] left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-full border border-[#8bdcff]/55 bg-[#06111ed9] px-[7%] py-[3%] text-[clamp(4px,1.15vw,8px)] font-black uppercase tracking-[.11em] text-[#d9f7ff] shadow-[0_0_12px_rgba(58,191,255,.24)]">{isRolling ? tr("UI_1122", "Rolling two dice…") : isEndTurn ? tr("UI_1123", "Tap stage to end turn") : tr("UI_1124", "Tap white stage to roll")}</span>
      </button>
  ...
  return (
    <section className={`shrink-0 rounded-lg border border-[#8ce5ff]/65 bg-[linear-gradient(145deg,#f2fbff,#b9e5f5_58%,#5e9ebc)] text-[#07141d] shadow-[inset_0_1px_0_white,0_0_14px_rgba(84,214,255,.22)] ${compact ? "mt-[3%] p-1" : "mt-2 p-1.5"}`}>
      ...
      <div className={compact ? "px-1 py-0.5" : "px-1.5 py-1"}>
        ...
        {compact ? <div className="mt-px grid grid-cols-3 gap-x-0.5 border-t border-[#174a60]/25 pt-px text-[clamp(3.5px,.68vw,4.5px)] font-black uppercase"><span><LocalizedText id="UI_1144" fallback={tr("UI_1144", "Rent")} />{currency.format(currentRent)}</span><span className="text-center"><LocalizedText id="UI_1142" fallback={tr("UI_1142", "Next")} />{currency.format(nextUpgradeCost)}</span><span className="text-right"><LocalizedText id="UI_1140" fallback={tr("UI_1140", "Mort.")} />{currency.format(getMortgageValue(space))}</span></div> : <div className="mt-1 grid grid-cols-2 gap-x-1 border-t border-[#174a60]/25 pt-1 text-[clamp(4.5px,.95vw,6px)] font-black uppercase"><span>{level === 0 ? `Landing rent ${currency.format(currentRent)}` : level === MAX_PROPERTY_LEVEL ? tr("UI_1106", "Hotel") : `${level} houses`}</span><span className="text-right">{level === MAX_PROPERTY_LEVEL ? tr("UI_1138", "Hotel complete") : `Next build ${currency.format(nextUpgradeCost)}`}</span><span>{level === MAX_PROPERTY_LEVEL ? tr("UI_1137", "Construction complete") : `Construction ${currency.format(nextUpgradeCost)}`}</span><span className="text-right"><LocalizedText id="UI_1141" fallback={tr("UI_1141", "Mortgage")} />{currency.format(getMortgageValue(space))}</span></div>}
      </div>
    </section>
  );
}
...
function RentPaymentSummary({ cityName, amount, message, compact = false }: { cityName: string; amount: number; message: string; compact?: boolean }) {
  return (
    <section className={`rounded-xl border border-[#7edbff]/70 bg-[linear-gradient(145deg,#103c59,#071522)] shadow-[inset_0_1px_0_rgba(255,255,255,.16),0_0_18px_rgba(51,183,255,.2)] ${compact ? "mt-[5%] p-1.5" : "mt-3 p-3"}`}>
      <p className={`font-black uppercase tracking-[.18em] text-[#8ee5ff] ${compact ? "text-[clamp(5px,1vw,7px)]" : "text-[clamp(6px,1.1vw,8px)]"}`}><LocalizedText id="UI_1145" fallback="Landing on" />{cityName}</p>
      ...
      <section className={`flex w-[92%] flex-col overflow-hidden rounded-[14px] border-2 text-center shadow-[0_0_24px_rgba(80,197,255,.32),0_12px_32px_rgba(0,0,0,.5)] ${isBlueAlert ? "h-[80%] border-[#69d1ff] bg-[radial-gradient(circle_at_50%_-15%,rgba(73,201,255,.2),transparent_42%),linear-gradient(145deg,#142d42,#060e17_58%,#102234)] p-[5%]" : "h-full border-[#ffe15b] bg-[linear-gradient(145deg,#26311e,#0a1018_52%,#1f1705)] p-[4%]"}`} role="dialog" aria-modal="true" aria-labelledby="monopoly-alert-title">
        ...
        {isPurchase ? (
          <div className="mt-2 grid shrink-0 grid-cols-2 gap-2 border-t border-white/15 pt-2">
            <button type="button" onClick={onBuy} className="rounded-lg bg-[linear-gradient(180deg,#fff071,#e7a411)] px-2 py-1.5 text-[clamp(7px,1.35vw,10px)] font-black uppercase text-[#241600]"><LocalizedText id="UI_1153" fallback={tr("UI_1153", "Buy")} />{currency.format(getPropertyPrice(space))}</button>
            <button type="button" onClick={onSkip} className="rounded-lg border border-white/30 bg-black/20 px-2 py-1.5 text-[clamp(7px,1.35vw,10px)] font-black uppercase text-white"><LocalizedText id="UI_0130" fallback={tr("UI_0130", "Skip")} /></button>
          </div>
        ) : isOwned ? (
          <div className="mt-5 grid shrink-0 grid-cols-2 gap-1.5">
            <button type="button" onClick={onUpgrade} className="rounded-lg bg-[linear-gradient(180deg,#b9ff78,#59b91c)] px-2 py-1.5 text-[clamp(6px,1.2vw,9px)] font-black uppercase text-[#142205]"><LocalizedText id="UI_1154" fallback={tr("UI_1154", "Upgrade next level")} /></button>
            <button type="button" onClick={onSell} className="rounded-lg bg-[linear-gradient(180deg,#ffb46e,#d95818)] px-2 py-1.5 text-[clamp(6px,1.2vw,9px)] font-black uppercase text-[#2b1103]"><LocalizedText id="UI_1155" fallback={tr("UI_1155", "Sell / Auction")} /></button>
          </div>
        ) : isAuction ? (
          <div className="mt-5 grid shrink-0 grid-cols-2 gap-1.5">
            <button type="button" onClick={onAwardAuction} className="rounded-lg bg-[linear-gradient(180deg,#fff071,#e7a411)] px-2 py-1.5 text-[clamp(6px,1.2vw,9px)] font-black uppercase text-[#241600]"><LocalizedText id="UI_1156" fallback={tr("UI_1156", "Award highest bid")} /></button>
            <button type="button" onClick={onDismiss} className="rounded-lg border border-white/30 bg-black/20 px-2 py-1.5 text-[clamp(6px,1.2vw,9px)] font-black uppercase text-white"><LocalizedText id="UI_1157" fallback={tr("UI_1157", "Cancel sale")} /></button>
          </div>
        ) : (
          <button type="button" onClick={onDismiss} className={`w-full shrink-0 rounded-lg border px-2 font-black uppercase text-white ${isBlueAlert ? "mt-3 border-[#7fd9ff]/80 bg-[#1a638d] py-1.5 text-[clamp(8px,1.75cqw,10px)]" : "mt-5 border-[#7fd9ff]/65 bg-[#1a638d] py-2 text-[clamp(7px,1.35vw,10px)]"}`}><LocalizedText id="UI_0126" fallback={tr("UI_0126", "Continue")} /></button>
        )}
      </section>
    </div>
  );
...
function ActionControlBar({ disabled, sellDisabled, onOpen }: { disabled: boolean; sellDisabled: boolean; onOpen: (kind: BoardActionKind) => void }) {
  ...
  return (
    <nav aria-label={tr("UI_1163", "Property actions")} className="mx-auto grid w-full max-w-md grid-cols-5 gap-1">
  ...
  return (
    <div onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="absolute inset-[2%] z-[56] grid place-items-center overflow-hidden rounded-[8%] border border-[#86dcff]/45 bg-[#02070de8] p-[2%] backdrop-blur-sm">
      <section className="flex h-full max-h-full w-full flex-col overflow-hidden rounded-xl border border-[#69d1ff] bg-[linear-gradient(145deg,#132b3f,#060e17_58%,#122338)] p-[3%] text-center shadow-[0_0_24px_rgba(80,197,255,.3)]" role="dialog" aria-modal="true" aria-labelledby="property-action-title">
        <div className="flex items-center justify-between gap-2"><div className="min-w-0 text-left"><p className="text-[clamp(5px,1vw,7px)] font-black uppercase tracking-[.2em] text-[#80d8ff]"><LocalizedText id="UI_1164" fallback="Property control" /></p><h2 id="property-action-title" className="text-[clamp(11px,2.5vw,16px)] font-black text-white">{heading}</h2></div><button type="button" onClick={onClose} className="grid h-5 w-5 shrink-0 place-items-center rounded-full border border-white/25 text-[10px] text-white">×</button></div>
        {panel.kind === "build" && <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">{ownedProperties.map((space) => { const level = getPropertyLevel(activePlayer, space.id); const nextLevel = level + 1; const upgradeCost = getUpgradeCost(space, nextLevel); const eligible = canBuildEvenly(activePlayer, space); const reason = !hasCompleteColorSet(activePlayer, space) ? "Full set required" : level >= MAX_PROPERTY_LEVEL ? "Hotel complete" : !eligible ? "Build lowest title first" : `${currency.format(upgradeCost)} for ${nextLevel === MAX_PROPERTY_LEVEL ? "HOTEL" : `HOUSE ${nextLevel}`}`; return <div key={space.id} className="flex items-center justify-between gap-1 rounded-md border border-white/10 bg-white/5 px-1.5 py-1 text-left"><span className="min-w-0"><span className="block truncate text-[clamp(5px,1.1vw,7px)] font-black text-white">{space.label}</span><span className="block text-[clamp(4px,.85vw,6px)] font-bold text-slate-300">{level === MAX_PROPERTY_LEVEL ? tr("UI_1129", "HOTEL") : `${level} HOUSES`} · {reason}</span></span><button type="button" disabled={!eligible || activePlayer.cash < upgradeCost} onClick={() => onBuild(space.id)} className="rounded bg-[#79d83f] px-1.5 py-1 text-[clamp(5px,1vw,7px)] font-black text-[#102307] disabled:opacity-35"><LocalizedText id="UI_1158" fallback="BUILD" /></button></div>; })}{ownedProperties.length === 0 && <p className="pt-4 text-[8px] font-bold text-slate-300"><LocalizedText id="UI_1171" fallback={tr("UI_1171", "No properties owned.")} /></p>}</div>}
        {panel.kind === "sell" && <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">{ownedProperties.filter((space) => getPropertyLevel(activePlayer, space.id) > 0).map((space) => { const level = getPropertyLevel(activePlayer, space.id); const eligible = canSellEvenly(activePlayer, space); const refund = Math.round(getUpgradeCost(space, level) * 0.5); const clearRefund = Math.round(Array.from({ length: MAX_PROPERTY_LEVEL }, (_, index) => getUpgradeCost(space, index + 1)).reduce((total, cost) => total + cost, 0) * 0.5); return <div key={space.id} className="rounded-md border border-white/10 bg-white/5 px-1.5 py-1 text-left"><div className="flex items-center justify-between gap-1"><span className="truncate text-[clamp(5px,1.1vw,7px)] font-black text-white">{space.label} · {level === MAX_PROPERTY_LEVEL ? tr("UI_1129", "HOTEL") : `H${level}`}</span><span className="text-[clamp(4px,.85vw,6px)] font-bold text-[#ffd38c]"><LocalizedText id="UI_1172" fallback="50% REFUND" /></span></div>{level === MAX_PROPERTY_LEVEL ? <div className="mt-1 grid grid-cols-2 gap-1"><button type="button" disabled={!eligible} onClick={() => onSellBuilding(space.id, "hotel")} className="rounded bg-[#ffb46e] px-1 py-1 text-[clamp(4px,.85vw,6px)] font-black text-[#2b1103] disabled:opacity-35"><LocalizedText id="UI_1175" fallback={tr("UI_1175", "HOTEL → 4H +")} />{currency.format(refund)}</button><button type="button" disabled={!eligible} onClick={() => onSellBuilding(space.id, "clear")} className="rounded border border-[#ffb46e]/60 px-1 py-1 text-[clamp(4px,.85vw,6px)] font-black text-[#ffe0bf] disabled:opacity-35"><LocalizedText id="UI_1173" fallback={tr("UI_1173", "CLEAR +")} />{currency.format(clearRefund)}</button></div> : <button type="button" disabled={!eligible} onClick={() => onSellBuilding(space.id, "single")} className="mt-1 rounded bg-[#ffb46e] px-1.5 py-1 text-[clamp(5px,1vw,7px)] font-black text-[#2b1103] disabled:opacity-35"><LocalizedText id="UI_1178" fallback={tr("UI_1178", "SELL 1 HOUSE +")} />{currency.format(refund)}</button>}</div>; })}{!ownedProperties.some((space) => getPropertyLevel(activePlayer, space.id) > 0) && <p className="pt-4 text-[8px] font-bold text-slate-300"><LocalizedText id="UI_1176" fallback={tr("UI_1176", "No houses or hotels to sell.")} /></p>}</div>}
        {panel.kind === "mortgage" && <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">{ownedProperties.map((space) => { const blocked = activePlayer.mortgagedSpaceIds.includes(space.id) || hasAnyBuildingsInColorSet(activePlayer, space); return <div key={space.id} className="flex items-center justify-between gap-1 rounded-md border border-white/10 bg-white/5 px-1.5 py-1 text-left"><span><span className="block text-[clamp(5px,1.1vw,7px)] font-black text-white">{space.label}</span><span className="block text-[clamp(4px,.85vw,6px)] font-bold text-slate-300">{activePlayer.mortgagedSpaceIds.includes(space.id) ? tr("UI_1179", "Already mortgaged") : hasAnyBuildingsInColorSet(activePlayer, space) ? tr("UI_1182", "Sell all color-set buildings first") : `Receive ${currency.format(getMortgageValue(space))}`}</span></span><button type="button" disabled={blocked} onClick={() => onMortgage(space.id)} className="rounded bg-[#78b4e8] px-1.5 py-1 text-[clamp(5px,1vw,7px)] font-black text-[#061525] disabled:opacity-35"><LocalizedText id="UI_1160" fallback="MORTGAGE" /></button></div>; })}</div>}
        {panel.kind === "redeem" && <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">{ownedProperties.filter((space) => activePlayer.mortgagedSpaceIds.includes(space.id)).map((space) => { const redeemCost = Math.ceil(getMortgageValue(space) * 1.1); return <div key={space.id} className="flex items-center justify-between gap-1 rounded-md border border-white/10 bg-white/5 px-1.5 py-1 text-left"><span><span className="block text-[clamp(5px,1.1vw,7px)] font-black text-white">{space.label}</span><span className="block text-[clamp(4px,.85vw,6px)] font-bold text-slate-300"><LocalizedText id="UI_1184" fallback="Principal + 10% =" />{currency.format(redeemCost)}</span></span><button type="button" disabled={activePlayer.cash < redeemCost} onClick={() => onRedeem(space.id)} className="rounded bg-[#8ce37b] px-1.5 py-1 text-[clamp(5px,1vw,7px)] font-black text-[#102307] disabled:opacity-35"><LocalizedText id="UI_1161" fallback="REDEEM" /></button></div>; })}{!ownedProperties.some((space) => activePlayer.mortgagedSpaceIds.includes(space.id)) && <p className="pt-4 text-[8px] font-bold text-slate-300"><LocalizedText id="UI_1183" fallback={tr("UI_1183", "No mortgaged properties.")} /></p>}</div>}
        {panel.kind === "trade" && trade && <div className="mt-2 min-h-0 flex-1 overflow-y-auto pr-0.5 text-left">{trade.awaitingConfirmation ? <div className="rounded-lg border border-[#ffe36b]/55 bg-[#241c08]/70 p-2 text-center"><p className="text-[8px] font-black text-[#ffe36b]"><LocalizedText id="UI_1200" fallback={tr("UI_1200", "WAITING FOR")} />{recipient?.username.toUpperCase()} <LocalizedText id="UI_1198" fallback={tr("UI_1198", "TO CONFIRM")} /></p><p className="mt-1 text-[7px] text-white"><LocalizedText id="UI_1188" fallback={tr("UI_1188", "Cash:")} />{currency.format(trade.offeredCash)} <LocalizedText id="UI_1193" fallback={tr("UI_1193", "offered /")} />{currency.format(trade.requestedCash)} requested</p><div className="mt-2 grid grid-cols-2 gap-1">{viewerId === trade.recipientId || !viewerId ? <button type="button" onClick={onConfirmTrade} className="rounded bg-[#8ce37b] py-1 text-[7px] font-black text-[#102307]"><LocalizedText id="UI_1705" fallback={tr("UI_1705", "ACCEPT")} /></button> : <span className="rounded bg-white/10 py-1 text-[7px] font-black text-slate-400"><LocalizedText id="UI_1186" fallback={tr("UI_1186", "AWAITING RECIPIENT")} /></span>}<button type="button" onClick={(viewerId === trade.recipientId || !viewerId) ? onDeclineTrade : onClose} className="rounded border border-white/25 py-1 text-[7px] font-black text-white">{(viewerId === trade.recipientId || !viewerId) ? tr("UI_0941", "DECLINE") : tr("UI_1187", "CANCEL")}</button></div></div> : <><p className="text-[6px] font-black uppercase text-[#80d8ff]"><LocalizedText id="UI_1189" fallback={tr("UI_1189", "Choose opponent")} /></p><div className="mt-1 flex gap-1 overflow-x-auto pb-1">{players.filter((player) => player.id !== activePlayer.id && !player.bankrupt).map((player) => <button type="button" key={player.id} onClick={() => onUpdateTrade({ recipientId: player.id, requestedPropertyId: null })} className={`shrink-0 rounded border px-1.5 py-1 text-[6px] font-black ${player.id === trade.recipientId ? "border-white bg-white/15" : "border-white/15"}`} style={{ color: player.color }}>{player.username}</button>)}</div><div className="mt-1 grid grid-cols-2 gap-1 text-[6px]"><div className="rounded border border-white/10 bg-white/5 p-1"><p className="font-black text-[#9ee5ff]"><LocalizedText id="UI_1201" fallback={tr("UI_1201", "YOU OFFER CASH")} /></p><div className="mt-1 flex items-center justify-between"><button type="button" onClick={() => onUpdateTrade({ offeredCash: Math.max(0, trade.offeredCash - 50) })}>−</button><span className="font-black text-white">{currency.format(trade.offeredCash)}</span><button type="button" onClick={() => onUpdateTrade({ offeredCash: Math.min(activePlayer.cash, trade.offeredCash + 50) })}>+</button></div></div><div className="rounded border border-white/10 bg-white/5 p-1"><p className="font-black text-[#ffd38c]"><LocalizedText id="UI_1202" fallback={tr("UI_1202", "YOU REQUEST CASH")} /></p><div className="mt-1 flex items-center justify-between"><button type="button" onClick={() => onUpdateTrade({ requestedCash: Math.max(0, trade.requestedCash - 50) })}>−</button><span className="font-black text-white">{currency.format(trade.requestedCash)}</span><button type="button" onClick={() => onUpdateTrade({ requestedCash: Math.min(recipient?.cash ?? 0, trade.requestedCash + 50) })}>+</button></div></div></div><p className="mt-1 text-center text-[6px] font-bold text-[#ffd38c]">A trade must include a title or Jail-Free card; cash-only requests are blocked.</p><div className="mt-1 grid grid-cols-2 gap-1 text-[6px]"><label className="rounded border border-white/10 bg-white/5 p-1"><span className="block font-black text-[#9ee5ff]"><LocalizedText id="UI_1192" fallback={tr("UI_1192", "OFFER TITLE")} /></span><select value={trade.offeredPropertyId ?? ""} onChange={(event) => onUpdateTrade({ offeredPropertyId: event.target.value || null })} className="mt-1 w-full bg-transparent text-white"><option value="">None</option>{offerableProperties.map((space) => <option key={space.id} value={space.id}>{space.label}</option>)}</select></label><label className="rounded border border-white/10 bg-white/5 p-1"><span className="block font-black text-[#ffd38c]"><LocalizedText id="UI_1196" fallback={tr("UI_1196", "REQUEST TITLE")} /></span><select value={trade.requestedPropertyId ?? ""} onChange={(event) => onUpdateTrade({ requestedPropertyId: event.target.value || null })} className="mt-1 w-full bg-transparent text-white"><option value="">None</option>{requestedProperties.map((space) => <option key={space.id} value={space.id}>{space.label}</option>)}</select></label></div><div className="mt-1 grid grid-cols-2 gap-1 text-[6px]"><button type="button" onClick={() => onUpdateTrade({ offeredJailFreeCard: !trade.offeredJailFreeCard })} disabled={activePlayer.jailFreeCards < 1} className={`rounded border p-1 font-black ${trade.offeredJailFreeCard ? "border-[#80d8ff] bg-[#0e3951]" : "border-white/15"}`}><LocalizedText id="UI_1191" fallback={tr("UI_1191", "OFFER JAIL-FREE")} /></button><button type="button" onClick={() => onUpdateTrade({ requestedJailFreeCard: !trade.requestedJailFreeCard })} disabled={(recipient?.jailFreeCards ?? 0) < 1} className={`rounded border p-1 font-black ${trade.requestedJailFreeCard ? "border-[#ffd38c] bg-[#4d2d0d]" : "border-white/15"}`}><LocalizedText id="UI_1195" fallback={tr("UI_1195", "REQUEST JAIL-FREE")} /></button></div><button type="button" onClick={onProposeTrade} className="mt-2 w-full rounded bg-[#74c8f5] py-1.5 text-[7px] font-black text-[#071826]"><LocalizedText id="UI_1194" fallback={tr("UI_1194", "PROPOSE TRADE")} /></button></>}</div>}
      </section>
    </div>
  );
}
...
function CornerPlayerCard({ player, active, empty }: { player?: Player; active?: boolean; empty?: boolean }) {
  if (empty || !player) {
    return <article className="portrait-player-card flex min-h-[54px] items-center justify-center rounded-xl border border-dashed border-white/15 bg-[#07101a]/45 px-2 text-center text-[8px] font-black uppercase tracking-[.14em] text-slate-500"><LocalizedText id="UI_1203" fallback="Open seat" /></article>;
  }
    ...
    <div className="min-h-[100dvh] overflow-hidden bg-[radial-gradient(circle_at_50%_26%,#163c5d_0%,#07111d_40%,#02060b_100%)] px-4 pb-4 pt-[var(--app-safe-top)] text-white">
      ...
      <div className="mx-auto flex min-h-[calc(100dvh-2rem)] w-full max-w-md flex-col">
        ...
        <div className="relative mt-5 h-48 overflow-hidden rounded-3xl border border-[#3c92cf]/55 bg-[#020812] shadow-[0_0_28px_rgba(42,151,255,.22)]">
          <Image src="/images/lobby-diamond-skyline.png" alt={tr("UI_1206", "Glowing diamond above a futuristic skyline")} fill priority sizes="(max-width: 768px) 100vw, 448px" className="object-cover object-center" />
          <div aria-hidden className="absolute inset-0 bg-[linear-gradient(180deg,rgba(1,8,19,.04),rgba(1,8,19,.18)_70%,rgba(1,8,19,.34))]" />
        </div>
        <div className="mt-5 text-center"><p className="text-[10px] font-black uppercase tracking-[.24em] text-[#7ecfff]"><LocalizedText id="UI_1209" fallback="Southeast Asia edition" /></p><h1 className="mt-1 text-3xl font-black tracking-tight"><LocalizedText id="UI_1208" fallback="Choose your players" /></h1><p className="mt-2 text-sm text-slate-300"><LocalizedText id="UI_1207" fallback="Choose 2–4 players. Every toy pawn starts together on" /><b className="text-[#ffdb32]"><LocalizedText id="UI_1069" fallback="GO" /></b>.</p></div>
        <div className="mt-6 grid grid-cols-3 gap-2">{[2, 3, 4].map((value) => <button key={value} type="button" onClick={() => onCountChange(value)} className={`rounded-2xl border px-2 py-3 text-center transition ${count === value ? "border-[#4ec3ff] bg-[#12639a]/50 shadow-[0_0_16px_rgba(63,184,255,.35)]" : "border-white/12 bg-white/5"}`}><span className="block text-2xl font-black">{value}</span><span className="mt-1 block text-[9px] font-black uppercase tracking-wider text-slate-300"><LocalizedText id="UI_0055" fallback="Players" /></span></button>)}</div>
        <div className="mt-4"><p className="text-center text-[9px] font-black uppercase tracking-[.16em] text-[#7ecfff]"><LocalizedText id="UI_1210" fallback="Final checkpoint" /></p><div className="mt-2 grid grid-cols-3 gap-2">{[30, 50, 100].map((value) => <button key={value} type="button" onClick={() => onRoundLimitChange(value)} className={`rounded-xl border py-2 text-xs font-black ${roundLimit === value ? "border-[#ffda38] bg-[#6f5310]/45 text-[#fff1ad]" : "border-white/12 bg-white/5 text-slate-300"}`}>{value} <LocalizedText id="UI_1212" fallback="ROUNDS" /></button>)}</div><p className="mt-2 text-center text-[10px] text-slate-400"><LocalizedText id="UI_1211" fallback="Play until one player remains. If no one is bankrupt at this checkpoint, the highest net worth wins." /></p></div>
        ...
        <button type="button" onClick={onStart} className="mt-auto rounded-2xl border-b-4 border-[#086ba5] bg-[linear-gradient(180deg,#4fc8ff,#0c82ca)] px-4 py-4 text-sm font-black tracking-[.16em] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.7),0_7px_18px_rgba(0,0,0,.4)] active:translate-y-px"><LocalizedText id="UI_1213" fallback="START GAME" /></button>
      </div>
    </div>
  );
}
...
export default function Monopoly({ onBack, onClose, userId, roomId }: MonopolyProps) {
  ...
  if (roomId && !roomReady) return <div className="fixed inset-0 grid place-items-center bg-[#07111c] text-white"><LocalizedText id="UI_1027" fallback="Loading shared Monopoly board…" /></div>;
  if (phase === "setup") {
    ...
    <div className="fixed inset-0 z-[100] h-[100dvh] min-h-[100svh] w-full overflow-hidden overscroll-none touch-none bg-[radial-gradient(circle_at_50%_33%,#173a58_0%,#07111c_52%,#02060b_100%)] font-sans text-white">
      ...
      <div className="mx-auto flex h-full min-h-0 w-full max-w-xl flex-col">
        <header className="portrait-header mx-2 mt-[var(--app-safe-top)] flex h-12 shrink-0 items-center rounded-xl border border-[#36556e] bg-[#0a131c]/95 px-2 shadow-[0_4px_14px_rgba(0,0,0,.45),inset_0_1px_0_rgba(255,255,255,.08)]">
          <button type="button" onClick={handleExit} aria-label={tr("UI_1028", "Exit Monopoly")} className="grid h-9 w-9 place-items-center rounded-lg transition hover:bg-white/10"><ArrowLeft className="h-5 w-5" /></button>
          <div className="min-w-0 flex-1 text-center"><p className="truncate text-[clamp(9px,2.7vw,13px)] font-black tracking-[.08em] text-[#eaf8ff]">{gameState.roundsLeft} / {gameState.roundLimit} <LocalizedText id="UI_1029" fallback="ROUNDS · LAST PLAYER TAKES THE POT" /></p><p className="mt-0.5 text-[7px] font-black tracking-[.2em] text-[#72caff]"><LocalizedText id="UI_1030" fallback="SOUTHEAST ASIA EDITION" /></p></div>
          <button type="button" onClick={() => setShowRules(true)} aria-label={tr("UI_1031", "Open rules")} className="grid h-9 w-9 place-items-center rounded-lg transition hover:bg-white/10"><ShieldCheck className="h-5 w-5 text-[#76cfff]" /></button>
        </header>
      ...
      {showRules && <div className="absolute inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><section className="w-full max-w-sm rounded-2xl border border-[#56bbff] bg-[linear-gradient(135deg,#12283b,#06101a)] p-5 shadow-[0_16px_44px_rgba(0,0,0,.65)]"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[.2em] text-[#72caff]"><LocalizedText id="UI_1040" fallback={tr("UI_1040", "Crystal table rules")} /></p><h2 className="mt-1 text-xl font-black"><LocalizedText id="UI_1043" fallback={tr("UI_1043", "Neon Monopoly")} /></h2></div><button type="button" onClick={() => setShowRules(false)} className="grid h-8 w-8 place-items-center rounded-full border border-white/20"><X className="h-4 w-4" /></button></div><ol className="mt-4 space-y-2 text-xs leading-relaxed text-slate-200"><li><b className="text-[#ffda2d]">1.</b> <LocalizedText id="UI_1041" fallback={tr("UI_1041", "Every player starts with their paid entry × 10 as in-game cash.")} /></li><li><b className="text-[#ffda2d]">2.</b> <LocalizedText id="UI_1045" fallback={tr("UI_1045", "Roll, build, trade, mortgage, and collect rent. A player who cannot pay is bankrupt and transfers their holdings.")} /></li><li><b className="text-[#ffda2d]">3.</b> <LocalizedText id="UI_1044" fallback={tr("UI_1044", "Passing GO adds")} />{currency.format(GO_SALARY)} <LocalizedText id="UI_1047" fallback={tr("UI_1047", "to the circulation balance.")} /></li><li><b className="text-[#ffda2d]">4.</b> <LocalizedText id="UI_1046" fallback={tr("UI_1046", "The last survivor takes the full circulation balance. At the selected checkpoint, highest net worth wins if multiple players remain. The winner receives 10% as points.")} /></li></ol><button type="button" onClick={() => setShowRules(false)} className="mt-5 w-full rounded-xl bg-gradient-to-b from-[#49c5ff] to-[#0c7fc4] py-3 text-xs font-black uppercase tracking-wider"><LocalizedText id="UI_1042" fallback={tr("UI_1042", "Got it")} /></button></section></div>}

      {winner && <div className="absolute inset-0 z-[60] grid place-items-center bg-black/80 p-4 backdrop-blur-sm"><section className="w-full max-w-sm rounded-2xl border-2 bg-[linear-gradient(135deg,#112b40,#061019)] p-6 text-center shadow-[0_0_40px_rgba(44,173,255,.22)]" style={{ borderColor: winner.color }}><Crown className="mx-auto h-10 w-10" style={{ color: winner.color }} fill="currentColor" /><p className="mt-2 text-[10px] font-black uppercase tracking-[.2em]" style={{ color: winner.color }}><LocalizedText id="UI_1053" fallback={tr("UI_1053", "Southeast Asia champion")} /></p><h2 className="mt-2 text-3xl font-black">{winner.username}</h2><p className="mt-2 text-sm text-slate-200"><LocalizedText id="UI_1050" fallback={tr("UI_1050", "Final points:")} />{gameState.finalPointsAwarded ? winner.points : getFinalPoints(winner)} · {currency.format(winner.cash)} <LocalizedText id="UI_1048" fallback={tr("UI_1048", "cash × 10%")} /></p><div className="mt-4 space-y-1 rounded-xl border border-white/10 bg-black/20 p-2 text-left">{gameState.players.filter((player) => !player.bankrupt).sort((first, second) => getFinalPoints(second) - getFinalPoints(first) || second.cash - first.cash).map((player) => <div key={player.id} className="flex items-center justify-between text-[10px] font-bold"><span style={{ color: player.color }}>{player.username}</span><span className="text-slate-100">{getFinalPoints(player)} <LocalizedText id="UI_1052" fallback="PTS ·" />{currency.format(player.cash)}</span></div>)}</div><button type="button" onClick={() => { turnEpochRef.current += 1; setSecondsLeft(TURN_DURATION_SECONDS); setGameState(createGameState(selectedPlayerCount)); }} className="mt-6 w-full rounded-xl bg-white/10 py-3 text-xs font-black uppercase tracking-wider transition hover:bg-white/20"><RotateCcw className="mr-1 inline h-4 w-4" /><LocalizedText id="UI_1051" fallback={tr("UI_1051", "Play again")} /></button><button type="button" onClick={() => setPhase("setup")} className="mt-2 w-full py-2 text-[10px] font-black uppercase tracking-wider text-[#77cbff]"><LocalizedText id="UI_1049" fallback={tr("UI_1049", "Change players")} /></button></section></div>}
    </div>
  );
}
