...
export default function BigTwoGame({onClose, onPlayAgain, roomId}:BigTwoGameProps) {
 ...
 if (roomId && !roomReady) return <div className="fixed inset-0 grid place-items-center bg-[#09090b] p-6 text-center text-white"><p className="font-bold"><LocalizedText id="UI_0372" fallback="Waiting for the shared Big Two deal…" /></p></div>;
 ...
 return <div className="fixed inset-0 z-[100] flex h-[100dvh] min-h-[100svh] min-h-0 flex-col overflow-hidden overscroll-none touch-none bg-[radial-gradient(circle_at_top,#14532d_0%,#052e2b_48%,#020617_100%)] text-white select-none">
   <header className="grid grid-cols-[1fr_auto_1fr] items-center border-b border-white/10 bg-slate-950/55 px-3 pb-2 pt-[var(--app-safe-top)]"><div>{onClose&&<button onClick={onClose} aria-label={tr("UI_0373", "Back to Arcade Hub")} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-slate-900"><span className="text-xl">←</span></button>}</div><div className="text-center"><h1 className="text-lg font-black text-amber-300"><LocalizedText id="UI_0374" fallback="BIG TWO" /></h1><p className="text-[9px] font-black uppercase tracking-widest text-emerald-200/70"><LocalizedText id="UI_0375" fallback="Classic · 4 Players" /></p></div><div className="flex justify-end gap-2"><button onClick={startGame} className="rounded-xl bg-amber-400 px-3 py-2 text-xs font-black text-slate-950"><LocalizedText id="UI_1468" fallback="New" /></button><button onClick={()=>setShowRules(true)} aria-label={tr("UI_0376", "How to play Big Two")} className="flex h-9 w-9 items-center justify-center rounded-full border border-[#ccff00] bg-slate-900 text-[#ccff00]"><span className="flex h-5 w-5 items-center justify-center rounded-full border border-[#ccff00] text-xs font-black">?</span></button></div></header>
   ...
   <main className="min-h-0 flex-1 p-2">
     <div className="relative mx-auto h-full w-full max-w-md overflow-hidden rounded-[2.2rem] border-4 border-emerald-300/25 bg-[radial-gradient(ellipse_at_center,#087443_0%,#075b37_52%,#06452e_100%)] shadow-[inset_0_0_45px_rgba(0,0,0,.35),0_18px_35px_rgba(0,0,0,.45)]">
       ...
       <section className="absolute left-24 right-24 top-[29%] flex min-h-40 flex-col items-center justify-center rounded-[1.75rem] border border-emerald-200/15 bg-emerald-950/25 p-2">
         <p className="mb-2 text-center text-[10px] font-black uppercase tracking-widest text-amber-200">{currentPlay ? `${playerNames[currentPlay.player]} · ${currentPlay.value.label}${freeLead ? tr("UI_0380", "· Free lead") : ""}` : tr("UI_0382", "New trick — any valid combination")}</p>
       ...
       <button onClick={()=>{if(roomId){void supabase.rpc("big_two_pass",{p_room_id:roomId}).then(({error})=>{if(error)setMessage(error.message);});}else passTurn(0);}} disabled={turn!==0||!currentPlay||freeLead||winner!==null} className="absolute bottom-[148px] left-4 z-30 flex h-14 w-14 items-center justify-center rounded-full border-2 border-amber-200 bg-gradient-to-b from-amber-300 to-amber-500 text-xs font-black text-amber-950 shadow-[0_6px_0_#9a5c00] active:translate-y-1 disabled:opacity-30"><LocalizedText id="UI_0383" fallback="PASS" /></button>
       <button onClick={handlePlay} disabled={turn!==0||winner!==null} className={`absolute bottom-[148px] right-4 z-30 flex h-14 w-14 items-center justify-center rounded-full border-2 text-xs font-black shadow-[0_6px_0_#14532d] active:translate-y-1 disabled:opacity-35 ${canPlay?"border-lime-200 bg-[#ccff00] text-slate-950":"border-slate-400 bg-slate-600 text-slate-200"}`}><LocalizedText id="UI_1665" fallback="PLAY" /></button>
       {hands[0].length === 1 && !oneCardCalled && winner===null && <button onClick={()=>{setOneCardCalled(true);setMessage("You called 1 Card!");if(roomId){void supabase.rpc("big_two_call_one",{p_room_id:roomId}).then(({error})=>{if(error){setOneCardCalled(false);setMessage(error.message);}});}}} className="absolute bottom-[148px] left-1/2 z-30 -translate-x-1/2 rounded-full border border-red-200 bg-red-600 px-3 py-2 text-[10px] font-black text-white shadow-lg"><LocalizedText id="UI_0384" fallback={tr("UI_0384", "CALL 1 CARD")} /></button>}
       {hands[0].length === 1 && oneCardCalled && winner===null && <div className="absolute bottom-[154px] left-1/2 z-30 -translate-x-1/2 whitespace-nowrap rounded-full border border-lime-200 bg-lime-400 px-3 py-2 text-[10px] font-black text-slate-950 shadow-lg"><LocalizedText id="UI_0385" fallback={tr("UI_0385", "✓ 1 CARD CALLED")} /></div>}

       <div className={`absolute bottom-0 left-0 right-0 z-20 ${turn===0?"drop-shadow-[0_0_8px_rgba(253,224,71,.5)]":""}`}>
         <div className="flex items-center justify-between px-4"><span className={`text-[10px] font-black uppercase ${turn===0?"text-amber-300":"text-cyan-200"}`}><LocalizedText id="UI_0387" fallback="Your hand ·" />{hands[0].length}</span><span className="text-[9px] text-emerald-100">{selectedValue?.label??tr("UI_0386", "Tap cards to select")}</span></div>
   ...
   {showRules&&<div className="fixed inset-x-0 bottom-20 top-14 z-[250] flex items-center justify-center bg-slate-950/88 p-4 backdrop-blur-md"><div className="max-h-[92%] w-full max-w-md overflow-y-auto rounded-[2rem] border-2 border-[#ccff00] bg-slate-900 p-5"><div className="flex justify-between"><div><p className="text-[10px] font-black uppercase tracking-widest text-[#ccff00]"><LocalizedText id="UI_0391" fallback={tr("UI_0391", "Classic Big Two")} /></p><h2 className="text-2xl font-black"><LocalizedText id="UI_0394" fallback={tr("UI_0394", "How to Play")} /></h2></div><button onClick={()=>setShowRules(false)} className="h-10 w-10 rounded-full bg-slate-800 text-2xl">×</button></div><div className="mt-5 space-y-3">{[["🃏",tr("UI_0390", "Card order"),tr("UI_0388", "3 is lowest and 2 is highest. Suits are ♦, ♣, ♥, ♠ from low to high.")],["♦",tr("UI_0396", "Opening play"),tr("UI_0400", "The player holding 3♦ starts and must include it in the first play.")],["✋",tr("UI_0401", "Valid plays"),tr("UI_0398", "Play a single, pair, triple, or five cards: straight, flush, full house, four of a kind plus one, or straight flush.")],["⬆️",tr("UI_0389", "Beat the table"),tr("UI_0395", "Match the number of cards. Five-card hands rank: straight, flush, full house, four of a kind, straight flush.")],["⏭️",tr("UI_0397", "Pass and reset"),tr("UI_0402", "You may pass. After all three opponents pass, the last player starts a new trick with any valid play.")],["🏆",tr("UI_0392", "Empty your hand"),tr("UI_0399", "The first player to play all 13 cards wins.")]].map(([icon,title,text])=><div key={title} className="flex gap-3 rounded-2xl border border-slate-700 bg-slate-800 p-3"><span className="text-xl">{icon}</span><div><h3 className="font-black text-amber-300">{title}</h3><p className="text-xs leading-5 text-slate-300">{text}</p></div></div>)}</div><button onClick={()=>setShowRules(false)} className="mt-5 w-full rounded-2xl bg-amber-400 py-3 font-black text-slate-950"><LocalizedText id="UI_0393" fallback={tr("UI_0393", "Got It — Let&apos;s Play")} /></button></div></div>}
   {winner!==null&&<div className="absolute inset-0 z-[260] flex items-center justify-center bg-slate-950/85 p-5 backdrop-blur-sm"><div className="w-full max-w-sm overflow-hidden rounded-[2rem] border-2 border-amber-300 bg-slate-900 text-center shadow-2xl"><div className="bg-[radial-gradient(circle_at_center,#fbbf24_0%,#92400e_45%,#111827_78%)] px-7 pb-6 pt-8"><div className="text-7xl drop-shadow-xl">{viewerWon || (!roomId && winner===0)?"🏆":"🎮"}</div><p className="mt-3 text-xs font-black uppercase tracking-[.25em] text-amber-100"><LocalizedText id="UI_0404" fallback={tr("UI_0404", "Big Two complete")} /></p><h2 className="mt-2 text-3xl font-black text-white">{viewerWon || (!roomId && winner===0)?tr("UI_0409", "You Win!"):`${winnerName || playerNames[winner] || tr("UI_0079", "Player")} Wins`}</h2><p className="mt-2 text-sm text-amber-100/90">{viewerWon || (!roomId && winner===0)?tr("UI_0405", "Excellent play — you cleared your hand first."):tr("UI_0408", "The match has ended. Ready for another round?")}</p></div><div className="space-y-3 p-5"><button onClick={onPlayAgain ?? startGame} className="w-full rounded-2xl bg-amber-400 py-3 font-black text-slate-950"><LocalizedText id="UI_0407" fallback={tr("UI_0407", "Play Again")} /></button><button onClick={onClose} className="w-full rounded-2xl border border-slate-600 bg-slate-800 py-3 font-black text-white"><LocalizedText id="UI_0406" fallback={tr("UI_0406", "Exit to Arcade")} /></button></div></div></div>}
 </div>;
}
