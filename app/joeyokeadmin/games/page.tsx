...
export default function GameCatalogManager() {
  ...
  // Group games by their category for rendering
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={fetchGames} className="flex items-center gap-2 bg-neutral-100 dark:bg-white/10 px-4 py-2 rounded-lg border border-neutral-200 dark:border-white/5 text-xs font-bold hover:bg-neutral-200 dark:hover:bg-white/20 transition-colors w-fit shadow-sm">
          <span className="material-symbols-outlined text-sm">refresh</span> Refresh Catalog
        </button>
      </header>

      {loading ? (
        <div className="py-12 text-center text-xs font-bold text-neutral-400 tracking-widest uppercase animate-pulse">
          Loading Catalog Data...
        </div>
      ) : Object.keys(groupedGames).length === 0 ? (
        <div className="py-12 text-center text-xs text-neutral-400 border border-dashed border-neutral-200 dark:border-white/10 rounded-2xl mt-6">
          ...
          {Object.entries(groupedGames).map(([category, categoryGames]) => (
            <div key={category}>
              ...
              <div className="flex items-center gap-3 mb-4 border-b border-neutral-200 dark:border-white/10 pb-2">
                <span className="material-symbols-outlined text-indigo-600 dark:text-[#c3f400]">
                  {category.includes("Strategy") ? "chess" : category.includes("Party") ? "celebration" : "sports_esports"}
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categoryGames.map((game) => (
                  <div key={game.id} className="bg-white dark:bg-[#111c33] border border-neutral-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm flex flex-col transition-transform hover:-translate-y-1">
                    <div className="p-5 border-b border-neutral-200 dark:border-white/5 flex items-start gap-4 relative">
